...
class CasosC4 extends Conexion
{
	...
	public function fn_registrar_caso_c4($nom_archivo_c4, $datos_exp_c4)
	{
		try {
			...
			$sql   = $conn->prepare("INSERT INTO 		tbl_c4_expedientes 
											...
											VALUES 		(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			$sql->bindParam(1, $datos_exp_c4['c4_codigo'], PDO::PARAM_STR);
			$sql->bindParam(2, $datos_exp_c4['c4_folio'], PDO::PARAM_STR);
			$sql->bindParam(3, $datos_exp_c4['c4_numero'], PDO::PARAM_STR, 30);
			$sql->bindParam(4, $datos_exp_c4['c4_no_oficio'], PDO::PARAM_STR, 30);
			$sql->bindParam(5, $nom_archivo_c4, PDO::PARAM_STR);
			$sql->bindParam(6, $datos_exp_c4['c4_fecha_inicio'], PDO::PARAM_STR);
			$sql->bindParam(7, $datos_exp_c4['c4_pais'], PDO::PARAM_STR);
			...
			if (isset($_SESSION['reportante']) and !empty($_SESSION['reportante'])) {
				...
				foreach ($_SESSION['reportante'] as $row) {
					$sql3   = $conn->prepare("INSERT INTO	tbl_c4_reportantes
													(
														c4_inst_reportante,
														c4_nom_reportante,
													...
													VALUES	(?,?,?,?,?)");
					$sql3->bindParam(1, $row["institucion"], PDO::PARAM_STR, 30);
					$sql3->bindParam(2, $row["reportante"], PDO::PARAM_STR);
					$sql3->bindParam(3, $exp_id_caso_reportado, PDO::PARAM_STR);
					$sql3->bindParam(4, $id_caso, PDO::PARAM_STR, 30);
					$sql3->bindParam(5, $nombre_creador, PDO::PARAM_STR, 30);
	...
	public function editar_caso_c4($id, $c4_codigo, $c4_folio, $c4_numero, $c4_no_oficio, $c4_fecha_inicio, 
	$c4_pais, $c4_edo, $c4_mun, $c4_mun_edo, $c4_dirigido, $c4_dg,$estatus_exp_c4)
	{
		$sql = $this->dbh->prepare("UPDATE 	tbl_c4_expedientes 
									SET		c4_codigo = ?,
											...
											c4_estatus_caso=?
									WHERE	id = ?");
		if ($sql->execute(array($c4_codigo, $c4_folio, $c4_numero, $c4_no_oficio, $c4_fecha_inicio, $c4_pais, 
		$c4_edo, $c4_mun, $c4_mun_edo, $c4_dirigido, $c4_dg,$estatus_exp_c4, $id))) {
			return 'editado';
		} else
			return 'error';
	}
	public function lista_casos_c4()
	{
		...
		$sql = $this->dbh->prepare("
									SELECT 	id,
											...
											c4_fecha_inicio,
											c4_estatus_caso,
											tbl_c4_expedientes.id_estado,
											tbl_c4_expedientes.c4_mun,
											cat_estados.estado,
											cat_municipios.municipio 
									FROM	((tbl_c4_expedientes 
								INNER JOIN	cat_estados 
										ON	tbl_c4_expedientes.id_estado = cat_estados.id_estado)
