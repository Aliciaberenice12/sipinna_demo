...
class CasosC4 extends Conexion
{
	...
	public function fn_registrar_caso_c4($c4_codigo, $c4_folio, $c4_numero, $c4_no_oficio, $archivo, $c4_fecha_inicio, $c4_pais, $c4_edo, $c4_mun, $c4_mun_edo, $c4_dirigido, $c4_dg, $nombre_creador)
	{
		try {
		...
		(c4_codigo,c4_folio,c4_numero,c4_no_oficio,c4_ruta_sol_oficio,c4_fecha_inicio,c4_pais,id_estado,c4_mun,c4_mun_edo,
		c4_dirigido,c4_dg,c4_exp_folio,c4_anio,c4_created_by) 
		values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			$sql->bindParam(1, $c4_codigo, PDO::PARAM_STR);
			$sql->bindParam(2, $c4_folio, PDO::PARAM_STR);
			$sql->bindParam(3, $c4_numero, PDO::PARAM_STR, 30);
			$sql->bindParam(4, $c4_no_oficio, PDO::PARAM_STR, 30);
			$sql->bindParam(5, $archivo, PDO::PARAM_STR);
			$sql->bindParam(6, $c4_fecha_inicio, PDO::PARAM_STR);
			$sql->bindParam(7, $c4_pais, PDO::PARAM_STR);
			$sql->bindParam(8, $c4_edo, PDO::PARAM_STR);
			$sql->bindParam(9, $c4_mun, PDO::PARAM_STR);
			$sql->bindParam(10, $c4_mun_edo, PDO::PARAM_STR, 15);
			$sql->bindParam(11, $c4_dirigido, PDO::PARAM_STR, 30);
			$sql->bindParam(12, $c4_dg, PDO::PARAM_STR, 30);
	...
	public function editar_caso_c4($id, $c4_codigo, $c4_folio, $c4_numero, $c4_no_oficio, $c4_fecha_inicio, $c4_pais, $c4_edo, $c4_mun, $c4_mun_edo, $c4_dirigido, $c4_dg)
	{
		$sql = $this->dbh->prepare("UPDATE 	tbl_c4_expedientes 
									SET		c4_codigo = ?,
											...
											c4_dg=?
									WHERE	id = ?");
		if ($sql->execute(array($c4_codigo,$c4_folio,$c4_numero,$c4_no_oficio,$c4_fecha_inicio,$c4_pais,$c4_edo,$c4_mun,$c4_mun_edo,$c4_dirigido,$c4_dg,$id))) {
			return 'ok';
		} else
			return 'error';
	}
	public function lista_casos_c4()
	{
		...
		select id,c4_folio,c4_exp_folio,c4_no_oficio,c4_mun_edo ,c4_fecha_inicio,c4_estatus_caso,
		tbl_c4_expedientes.id_estado,tbl_c4_expedientes.c4_mun,cat_estados.estado,cat_municipios.municipio 
		from ((tbl_c4_expedientes 
		INNER JOIN cat_estados ON tbl_c4_expedientes.id_estado = cat_estados.id_estado)
	...
	public function lista_reportante_c4($fol_c4)
	{
		$sql = $this->dbh->prepare("
										SELECT	id_reportante_c4,
												c4_inst_reportante,
												c4_nom_reportante,
												c4_exp_folio_reportante,
												c4_reportante_id_caso,
												tbl_c4_reportantes.c4_reportante_id_caso,
												tbl_c4_desc_casos.c4_exp_id_caso
										FROM 	(tbl_c4_reportantes
									INNER JOIN 	tbl_c4_desc_casos 
										ON 		tbl_c4_reportantes.c4_reportante_id_caso = tbl_c4_desc_casos.c4_exp_id_caso)
										WHERE	c4_exp_folio_reportante = ?
									");
	...
	public function insertar_reportante_c4($nombre_creador)
	{
		try {
			$cok            = 0;
			$id = $this->obtener_folio('tbl_c4_expedientes', 'id');
			$exp_id_caso_reportado = 'C4/DG/' . $id . '/' . date('Y');
			$id_caso = $id;
			foreach ($_SESSION['reportante'] as $row) {
				$sql   = $this->dbh->prepare("INSERT INTO 	tbl_c4_reportantes
															(
															c4_inst_reportante,
															c4_nom_reportante,
															c4_exp_folio_reportante,
													...
													VALUES	(?,?,?,?,?)");
				$sql->bindParam(1, $row["institucion"], PDO::PARAM_STR, 30);
				$sql->bindParam(2, $row["reportante"], PDO::PARAM_STR);
				$sql->bindParam(3, $exp_id_caso_reportado, PDO::PARAM_STR);
				$sql->bindParam(4, $id_caso, PDO::PARAM_STR, 30);
				$sql->bindParam(5, $nombre_creador, PDO::PARAM_STR, 30);
				$cok++;
				if ($sql->execute()) {
					$cok++;
					$estatus = 'ok';
	...
	public function editar_reportante_c4($can_nom_rep_c4, $can_inst_rep_c4,$nombre_creador,$id)
	{
		$sql = $this->dbh->prepare("UPDATE 	tbl_c4_reportantes
		 								SET c4_nom_reportante=?,
											c4_inst_reportante=?,
											c4_update_by=?
											where id_reportante_c4= ?");
		if ($sql->execute(array( $can_nom_rep_c4, $can_inst_rep_c4,$nombre_creador,$id))) {
			return 'ok';
		} else
			return 'error';
	}
	public function obtener_dato_reportante_c4($id)
	{
			$sql = $this->dbh->prepare("SELECT 	id_reportante_c4,
												c4_inst_reportante,
												c4_nom_reportante
									FROM 		tbl_c4_reportantes 
									WHERE 		id_reportante_c4 = ?"
	...
	public function insertar_victima_c4($nombre_creador)
	{
		try {
			...
			foreach ($_SESSION['victima_c4'] as $row) {
				...
				$sql->bindParam(2, $row["c4_nom_vic"], PDO::PARAM_STR, 50);
				$sql->bindParam(3, $row["c4_num_delitos"], PDO::PARAM_STR, 2);
	...
	public function fn_lista_victimas_c4($fol_c4)
	{
		$sql = $this->dbh->prepare("
									...
									INNER JOIN 	tbl_c4_delitos_victimas 
										ON 		tbl_c4_victimas.c4_id_desc_caso = tbl_c4_delitos_victimas.c4_id_victima)
									INNER JOIN	tbl_c4_der_vul_victima
										ON		tbl_c4_victimas.c4_id_desc_caso = tbl_c4_der_vul_victima.c4_id_victima)
	...
	public function editar_delito_victima_c4($id_del_victima, $delito, $nombre_creador)
	{
		...
		$estatus2 = '';
		try {
			$sql = $conn->prepare("UPDATE 	tbl_c4_delitos_victimas
									SET 	c4_delito=?,
											c4_update_by=?
	...
	public function editar_derecho_victima_c4($id_der_victima_c4, $derecho_c4, $nombre_creador)
	{
		...
		$estatus2 = '';
		try {
			$sql = $conn->prepare("UPDATE 	tbl_c4_der_vul_victima
	...
	{
			$sql = $this->dbh->prepare("SELECT 	id_pro_responsable,
												c4_edad_responsable,
												c4_nom_responsable,
												c4_parentesco
									FROM 		tbl_c4_probable_responsable 
